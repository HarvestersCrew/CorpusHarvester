...
void test_logger_set_output_path() {
  try {
    ...
    Assertion::assert_throw(__FUNCTION__, "logger_exception");
  ...
  try {
    logger::set_output_path("/tmp/ultrarandompath/");
    Assertion::assert_throw(__FUNCTION__, "logger_exception");
  } catch (const logger_exception &e) {
  }
...
void test_logger_output_log() {
  ...
  ifstream f(logger::get_output_path());
  result << f.rdbuf();
  f.close();
...
void test_logger_start() {
  ...
  ifstream f1(logger::get_output_path());
  result << f1.rdbuf();
  f1.close();
...
void logger_test() {
  ...
  Assertion::test(test_logger_get_output_path, "test_logger_get_output_path");
