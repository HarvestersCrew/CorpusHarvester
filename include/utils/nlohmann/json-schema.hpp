...
#if (NLOHMANN_JSON_VERSION_MAJOR * 10000 + NLOHMANN_JSON_VERSION_MINOR * 100 + \
     NLOHMANN_JSON_VERSION_PATCH) < 30600
#error "Please use this library with NLohmann's JSON version 3.6.0 or higher"
#endif
#else
#error                                                                         \
    "expected existing NLOHMANN_JSON_VERSION_MAJOR preproc variable, please update to NLohmann's JSON 3.6.0"
#endif
...
public:
  ...
  // resolves relative changes (pathes or pointers) and resets part if proto or
  // hostname changes
...
typedef std::function<void(const json_uri & /*id*/, json & /*value*/)>
    schema_loader;
typedef std::function<void(const std::string & /*format*/,
...
/**
 * Checks validity of JSON schema built-in string format specifiers like
 * 'date-time', 'ipv4', ...
 */
...
public:
  ...
  json_validator(const json &, schema_loader = nullptr,
                 format_checker = nullptr);
  ~json_validator();
  ...
  // validate a json-document based on the root-schema with a custom
  // error-handler
