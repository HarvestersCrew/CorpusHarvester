...
public:
  ...
  void insert(const json_uri &uri, const std::shared_ptr<schema> &s) {
    ...
    if (schema != file.schemas.end() &&
        !(file.schemas.key_comp()(uri.fragment(), schema->first))) {
  ...
  std::shared_ptr<schema> get_or_create_ref(const json_uri &uri) {
    ...
    if (r != file.unresolved.end() &&
        !(file.unresolved.key_comp()(uri.fragment(), r->first))) {
      return r->second; // unresolved, already seen previously - use existing
                        // reference
    } else {
      return file.unresolved
          .insert(r, {uri.fragment(),
  ...
  void validate(const json::json_pointer &ptr, const json &instance,
    ...
    if (maximum_.first)
      if ((exclusiveMaximum_ && value >= maximum_.second) ||
        ...
        e.error(ptr, instance,
                "instance exceeds maximum of " +
                    std::to_string(maximum_.second));

    if (minimum_.first)
      if ((exclusiveMinimum_ && value <= minimum_.second) ||
          value < minimum_.second)
        e.error(ptr, instance,
                "instance is below minimum of " +
...
type_schema::make(json &schema, json::value_t type, root_schema *root,
                  const std::vector<nlohmann::json_uri> &uris,
  ...
  for (auto &uri : uris) { // for all URIs this schema is referenced by
    ...
    if (schema.type() == json::value_t::object)
      for (auto &u : schema.items())
        root->insert_unknown_keyword(
            uri, u.key(),
            u.value()); // insert unknown keywords for later reference
  }
  return sch;
}
...
json_validator::json_validator(const json &schema, schema_loader loader,
