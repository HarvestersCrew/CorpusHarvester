  ...
  if (location.size()) { // a location part has been found
    ...
    if (location.find("urn:") == 0) {
      ...
      if (proto != std::string::npos) { // extract the protocol
        ...
        if (authority !=
            std::string::npos) { // and the hostname (no proto without hostname)
      ...
      if (path[0] == '/') // if it starts with a / it is root-path
        path_ = path;
               ...
               0) { // the URL contained only a path and the current path has no
                    // / at the end, strip last element until / and append
