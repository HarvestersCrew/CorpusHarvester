...
/**
 * Many of the RegExes are from @see
...
void range_check(const T value, const T min, const T max) {
  if (!((value >= min) && (value <= max))) {
    ...
    out << "Value " << value << " should be in interval [" << min << "," << max
        << "] but is not!";
...
void rfc3339_time_check(const std::string &value) {
  ...
  /**
   * @todo Could be made more exact by querying a leap second database and
   * choosing the correct maximum in {58,59,60}. This current solution might
   * match some invalid dates but it won't lead to false negatives. This only
   * works if we know the full date, however
   */
...
void rfc3339_date_time_check(const std::string &value) {
  ...
  if (!std::regex_match(value, matches, dateTimeRegex)) {
    throw std::invalid_argument(
        value + " is not a date-time string according to RFC 3339.");
  }
...
const std::string decOctet{
    R"((?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))"}; // matches numbers 0-255
 ...
 *                     obs-dtext          ;  "[", "]", or "\"
 * @endverbatim
 * @todo Currently don't have a working tool for this larger ABNF to generate a
 * regex. Other options:
 *         - https://github.com/ldthomas/apg-6.3
 *         - https://github.com/akr/abnf
 *
 * The problematic thing are the allowed whitespaces (even newlines) in the
 * email. Ignoring those and starting with
 * @see
 * https://stackoverflow.com/questions/13992403/regex-validation-of-email-addresses-according-to-rfc5321-rfc5322
 * and trying to divide up the complicated regex into understandable ABNF
 * definitions from rfc5322 yields:
 */
...
const std::string domainLiteral{R"(\[(?:(?:)" + decOctet + R"()\.){3}(?:)" +
                                decOctet + R"(|[A-Za-z0-9-]*[A-Za-z0-9]:(?:)" +
                                dtext + "|" + obsqp + R"()+)\])"};

const std::string dotAtom{"(?:" + atext + R"(+(?:\.)" + atext + "+)*)"};
const std::string stackoverflowMagicPart{
    R"((?:[[:alnum:]](?:[[:alnum:]-]*[[:alnum:]])?\.)+)"
    R"([[:alnum:]](?:[[:alnum:]-]*[[:alnum:]])?)"};
const std::string email{"(?:" + dotAtom + "|" + quotedString + ")@(?:" +
                        stackoverflowMagicPart + "|" + domainLiteral + ")"};
} // namespace

namespace nlohmann {
namespace json_schema {
/**
 * Checks validity for built-ins by converting the definitions given as ABNF in
 * the linked RFC from
 * @see
 * https://json-schema.org/understanding-json-schema/reference/string.html#built-in-formats
 * into regular expressions using @see https://www.msweet.org/abnf/ and some
 * manual editing.
 *
 * @see https://json-schema.org/latest/json-schema-validation.html
 */
void default_string_format_check(const std::string &format,
  ...
  if (format == "date-time") {
    ...
    if (!std::regex_match(value, emailRegex)) {
      throw std::invalid_argument(
          value + " is not a valid email according to RFC 5322.");
    }
  } else if (format == "hostname") {
    static const std::regex hostRegex{hostname};
    if (!std::regex_match(value, hostRegex)) {
      throw std::invalid_argument(
          value + " is not a valid hostname according to RFC 3986 Appendix A.");
    }
  } else if (format == "ipv4") {
    const static std::regex ipv4Regex{"^" + ipv4Address + "$"};
    if (!std::regex_match(value, ipv4Regex)) {
      throw std::invalid_argument(
          value + " is not an IPv4 string according to RFC 2673.");
    }
  } else if (format == "ipv6") {
    static const std::regex ipv6Regex{ipv6Address};
    if (!std::regex_match(value, ipv6Regex)) {
      throw std::invalid_argument(
          value + " is not an IPv6 string according to RFC 5954.");
    }
    ...
    static const std::vector<std::string> jsonSchemaStringFormatBuiltIns{
        ...
        "regex"};
    if (std::find(jsonSchemaStringFormatBuiltIns.begin(),
                  jsonSchemaStringFormatBuiltIns.end(),
                  format) != jsonSchemaStringFormatBuiltIns.end()) {
      throw std::logic_error(
          "JSON schema string format built-in " + format +
          " not yet supported. " +
